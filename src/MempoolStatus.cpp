...
MempoolStatus::read_mempool()
{
    ...
    for (size_t i = 0; i < mempool_tx_info.size(); ++i)
    {
        ...
        double payed_for_kB = WAZN_AMOUNT(_tx_info.fee) / tx_size;
        ...
        last_tx.fee_micro_str    = wazneg::wazn_amount_to_str(_tx_info.fee, "{:0.4f}", false);
        last_tx.payed_for_kB_str = fmt::format("{:0.4f}", payed_for_kB);
        last_tx.payed_for_kB_micro_str = fmt::format("{:0.4f}", payed_for_kB);
...
bf::path MempoolStatus::blockchain_path {"/var/lib/wazn/lmdb"};
